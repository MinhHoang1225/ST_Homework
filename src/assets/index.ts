import bgImageSmall from "./images/bg-today-small.svg";
import bgImageLarge from "./images/bg-today-large.svg";
import Logo from "./images/logo.svg";
import UnitsIcon from "./images/icon-units.svg";
import DropdownIcon from "./images/icon-dropdown.svg";
import CheckmarkIcon from "./images/icon-checkmark.svg";
import SearchIcon from "./images/icon-search.svg";
import SunnyIcon from "./images/icon-sunny.webp";
import PartlyCloudyIcon from "./images/icon-partly-cloudy.webp";
import CloudyIcon from "./images/icon-partly-cloudy.webp";
import OvercastIcon from "./images/icon-overcast.webp";
import DrizzleIcon from "./images/icon-drizzle.webp";
import RainIcon from "./images/icon-rain.webp";
import StormIcon from "./images/icon-storm.webp";
import SnowIcon from "./images/icon-snow.webp";
import FogIcon from "./images/icon-fog.webp";
import DefaultIcon from "./images/icon-sunny.webp";
import RetryIcon from "../assets/images/icon-loading.svg"; 

export {
  bgImageSmall,
  bgImageLarge,
  Logo,
  UnitsIcon,
  DropdownIcon,
  CheckmarkIcon,
  SearchIcon,
  SunnyIcon,
  PartlyCloudyIcon,
  CloudyIcon,
  OvercastIcon,
  DrizzleIcon,
  RainIcon,
  StormIcon,
  SnowIcon,
  FogIcon,
  DefaultIcon,
    RetryIcon,
};